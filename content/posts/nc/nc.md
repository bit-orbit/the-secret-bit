...
### مفاهیم پیشنیاز
چنتا مفهوم وجود دارد که احتمالا بلدش هستید، اما بد نیست یک مروری به این مفاهیم داشته باشیم.
...
> و یا
> `192.168`
> و خب اگر شما نیاز دارید تا شنود محدود به ای‌پی لوکال نشود کافیست ای‌پی را به این ای‌پی تنظیم کنید
> `0.0.0.0`
...
اول باید ای‌پی گوگل را بدست بیاورید، برای اینکار با دستور 
`nslookup google.com`
ای‌پی گوگل را می گیریم و بعد آن را به 
`netcat`
می‌دهیم تا چک کند که آیا پورت 80 این ای‌پی باز است یا خیر.

```bash
nc -nv 142.250.185.142 80
```

خروجی این دستور قطعا به این صورت است
```bash
Connection to 142.250.185.142 80 port [tcp/*] succeeded!
```

اما اگر شما می‌خواید یک محدوده خاصی از پورت های یک سرور را چک کنید باید از سوییچ
`-z`
استفاده کنید.
